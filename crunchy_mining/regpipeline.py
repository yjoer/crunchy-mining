...
def validate_random_forest(train_val_sets: dict, encoders: object):    
    with mlflow.start_run(run_name="Random Forest"):
        for name, (X_train, y_train, X_val, y_val) in train_val_sets.items():
             with mlflow.start_run(run_name=name, nested=True):
                with trace_memory() as trace:
                    rf_reg = train_random_forest(X_train, y_train)
        
                y_pred = rf_reg.predict(X_val)
                result = encoders['y_min_max'].inverse_transform(y_pred.reshape(-1,1))
                mlflow.log_metrics(evaluate_regression(y_val,result))
                mlflow.log_metric("peak_memory_usage", trace["peak"])
...
def validate_decision_tree(train_val_sets: dict, encoders: object):    
    with mlflow.start_run(run_name="Decision Tree"):
        for name, (X_train, y_train, X_val, y_val) in train_val_sets.items():
             with mlflow.start_run(run_name=name, nested=True):
                with trace_memory() as trace:
                    dt_reg = train_decision_tree(X_train, y_train)
...
def validate_ridge_regression(train_val_sets: dict, encoders: object):    
    with mlflow.start_run(run_name="Ridge Regression"):
        for name, (X_train, y_train, X_val, y_val) in train_val_sets.items():
             with mlflow.start_run(run_name=name, nested=True):
                with trace_memory() as trace:
                    ridge_reg = train_ridge_reg(X_train, y_train)
...
def validate_xg_boost(train_val_sets: dict, encoders: object):    
    with mlflow.start_run(run_name="XGBoost"):
        for name, (X_train, y_train, X_val, y_val) in train_val_sets.items():
             with mlflow.start_run(run_name=name, nested=True):
                with trace_memory() as trace:
                    xg_boost = train_xg_boost(X_train, y_train)
